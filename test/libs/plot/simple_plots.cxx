...
protected:
	...
	//cgv::gui::gui_group_ptr ap;
	//cgv::gui::gui_group_ptr bp;
	//cgv::gui::gui_group_ptr pp;
	std::map<void*, std::string> file_path_map;
	cgv::plot::plot1d p1;

	std::vector<dataset*> datasets;

	std::vector<int> column_selection;
...
public:
	...
	{
		while (p1.get_nr_sub_plots() > 0)
			p1.delete_sub_plot(0);
		for (size_t c = 0; c < dp->data.size(); ++c) {
			if (column_selection[c] != 0) {
				unsigned pi = p1.add_sub_plot(dp->headers[c]);
				p1.ref_sub_plot_samples(pi).resize(dp->data[c].size());
				for (size_t i = 0; i < dp->data[c].size(); ++i)
					p1.ref_sub_plot_samples(pi)[i] = cgv::plot::plot1d::P2D((float)(i+1), (float)dp->data[c][i]);
			}
		}
		adjust_axes();
		post_redraw();
	...
	void construct_multi_plot(const std::vector<dataset*> dps)
	{
		while (p1.get_nr_sub_plots() > 0)
			p1.delete_sub_plot(0);
		for (size_t c = 0; c < dps[0]->data.size(); ++c) {
			if (column_selection[c] != 0) {
				unsigned pi = p1.add_sub_plot(dps[0]->headers[c]);
				p1.ref_sub_plot_strips(pi).resize(dps.size());
				size_t fst = 0;
				for (size_t i = 0; i < dps.size(); ++i) {
					p1.ref_sub_plot_strips(pi)[i] = dps[i]->data[c].size();

					p1.ref_sub_plot_samples(pi).resize(fst + dps[i]->data[c].size());
					for (size_t j = 0; j < dps[i]->data[c].size(); ++j)
						p1.ref_sub_plot_samples(pi)[fst+j] = cgv::plot::plot1d::P2D((float)(j+1), (float)dps[i]->data[c][j]);

					fst += dps[i]->data[c].size();
				}
			}
		}
		adjust_axes();
	...
	{
		//
		unsigned nr_cols = dp->headers.size();
		for (unsigned j = 0; j < 6; ++j) {
			if (j < nr_cols) {
				headers[j] = dp->headers[j];
				if (find_control(column_selection[j])) {
					find_control(column_selection[j])->set("label", dp->headers[j]);
					find_control(column_selection[j])->set("active", true);
				}
			}
			else {
				headers[j] = "";
				if (find_control(column_selection[j])) {
					find_control(column_selection[j])->set("label", "");
					find_control(column_selection[j])->set("active", false);
				}
			}
		}
		...
		for (unsigned c = 0; c < nr_cols; ++c)
			if (column_selection[c] != 0)
				++nr_selected;
		if (nr_selected == 0) {
			column_selection[0] = 1;
			update_member(&column_selection[0]);
		}
	...
	void on_set(void* member_ptr)
	{
		...
		if (member_ptr == &selected_dataset) {
			if (selected_dataset >= 0 && selected_dataset < datasets.size()) {
				update_gui(datasets[selected_dataset]);
				construct_plot(datasets[selected_dataset]);
			}
		}
		for (size_t c = 0; c < column_selection.size(); ++c) {
			if (member_ptr == &column_selection[c]) {
				construct_plot(datasets[selected_dataset]);
				post_redraw();
			}
		}
		...
		if (select) {
			if (b->cast<button>()) {
				...
				if (read_directory(file_path_map[&*(b->cast<gui_group>())]) > 0) {
					if (selected_dataset > datasets.size())
						selected_dataset = 0;
					update_gui(datasets[selected_dataset]);
