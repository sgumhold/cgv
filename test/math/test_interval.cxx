...
bool test_interval()
{
	...
	TEST_ASSERT_EQ(I1.intersection(I2).get_lower_bound(), -0.9f);
	TEST_ASSERT_EQ(I1.intersection(I2).get_upper_bound(), 0.3f);
	TEST_ASSERT_EQ(I1*2.0f, interval<float>(-1.8f, 2.6f));
	TEST_ASSERT(fabs((I1*I2).intersect(interval<float>(-2.47f, 1.71f)).get_size() - (I1*I2).get_size()) < 1e-6f);
	return true;
}
