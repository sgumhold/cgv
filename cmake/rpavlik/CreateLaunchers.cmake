...
macro(_launcher_process_args)
	...
	#### Added by Joachim Staib
	foreach(_config ${CONFIGS})
		list(APPEND _val_args "ARGS_${_config}")
	endforeach()
	#####
...
macro(_launcher_produce_vcproj_user)
	if(MSVC)
		...
		foreach(USERFILE_CONFIGNAME ${CONFIGS})

			get_target_property(USERFILE_${USERFILE_CONFIGNAME}_COMMAND
				${_targetname}
				EXECUTE_LOCATION_${USERFILE_CONFIGNAME})
		
			if (NOT USERFILE_${USERFILE_CONFIGNAME}_COMMAND)
				get_target_property(USERFILE_${USERFILE_CONFIGNAME}_COMMAND
					${_targetname}
			...
			##### Modification by Joachim Staib
			set(USERFILE_COMMAND_ARGUMENTS "${ARGS} ${ARGS_${USERFILE_CONFIGNAME}}")
			set(LAUNCHERSCRIPT_COMMAND_ARGUMENTS "${ARGS} ${FWD_ARGS}")
			######
	...
	if(CMAKE_CONFIGURATION_TYPES)
		...
		foreach(_config ${CONFIGS})
			get_target_property(USERFILE_${_config}_COMMAND
				${_targetname}
				EXECUTE_LOCATION_${_config})
			if (NOT USERFILE_${_config}_COMMAND)
				get_target_property(USERFILE_${_config}_COMMAND
					${_targetname}
					LOCATION_${_config})			
			endif()
		...
		get_target_property(USERFILE_COMMAND
			${_targetname}
			EXECUTE_LOCATION_${CMAKE_BUILD_TYPE})
