...
vec2 cfade(vec2 t) { return t * t * t * (t * (t * 6.0 - 15.0) + 10.0); }
vec3 cfade(vec3 t) { return t * t * t * (t * (t * 6.0 - 15.0) + 10.0); }
...
float cnoise(vec2 P) 
{
	...
	vec4 fx = Pf.xzxz;
	vec4 fy = Pf.yyww;
	vec4 i = cpermute(cpermute(ix) + iy);
	...
	vec2 cfade_xy = cfade(Pf.xy);
	vec2 n_x = mix(vec2(n00, n01), vec2(n10, n11), cfade_xy.x);
	float n_xy = mix(n_x.x, n_x.y, cfade_xy.y);
	return 2.3 * n_xy;
}
float cnoise(vec3 P) 
{
	...
	vec4 norm0 = ctaylorInvSqrt(vec4(dot(g000, g000), dot(g010, g010), dot(g100, g100), dot(g110, g110)));
	g000 *= norm0.x;
	g010 *= norm0.y;
	g100 *= norm0.z;
	g110 *= norm0.w;
	vec4 norm1 = ctaylorInvSqrt(vec4(dot(g001, g001), dot(g011, g011), dot(g101, g101), dot(g111, g111)));
	g001 *= norm1.x;
	g011 *= norm1.y;
	g101 *= norm1.z;
	g111 *= norm1.w;
	...
	vec3 cfade_xyz = cfade(Pf0);
	vec4 n_z = mix(vec4(n000, n100, n010, n110), vec4(n001, n101, n011, n111), cfade_xyz.z);
	vec2 n_yz = mix(n_z.xy, n_z.zw, cfade_xyz.y);
	float n_xyz = mix(n_yz.x, n_yz.y, cfade_xyz.x);
	return 2.2 * n_xyz;
}
float cnoise(vec4 P) {
