	...
	void stream_vis_context::parse_declarations(const std::string& declarations)
	{
		...
		if (reader.parse_declarations()) {
			...
			for (const auto& vi : view_infos) {
				...
				if (v_overlay) {
					v_overlay->set_stretch_mode(cgv::overlay::StretchMode::kPercentual, vi.stretch);
					v_overlay->set_alignment(cgv::overlay::Alignment::kPercentual, cgv::overlay::Alignment::kPercentual, vi.offset);
