...
namespace cgv {
	namespace math {
		...
		/*! Input is the number of points \c nr_points and a pointer \c _points to the point array. Points
			...
			fit are written to 3 floats pointed to by \c _evals. If \c _evecs is specified, also the eigenvectors from the 
			fit are written in 3 float trippels to memory pointed to by \c _evecs.*/
		extern CGV_API void estimate_normal_ls(unsigned nr_points, const float* _points, float* _normal, float* _evals = 0, float* _evecs = 0);

		/// Weighted version of \c estimate_normal_ls with additional input \c _weights pointing to \c nr_points scalar weights.
		extern CGV_API void estimate_normal_wls(unsigned nr_points, const float* _points, const float* _weights, float* _normal, float* _evs = 0, float* _evecs = 0);
	}
}
