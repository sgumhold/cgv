...
public:
	...
	void create(const std::string& fmt_dcr, ptrdiff_t w=-1, ptrdiff_t h=-1);
	void clear(double v);
	template <typename T>
	...
	void copy_rectangle(const image_view& src, ptrdiff_t X = 0, ptrdiff_t Y = 0, ptrdiff_t x = 0, ptrdiff_t y = 0, ptrdiff_t w = -1, ptrdiff_t h = -1);
	void sub_sample();
};
