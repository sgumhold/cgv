...
struct object_collection {
    ...
    void add_object(base_ptr object) { objects.push_back(object); }
...
void enable_registration_debugging() { ref_registration_debugging_enabled() = true; }
/// disable registration debugging
void disable_registration_debugging() { ref_registration_debugging_enabled() = false; }

/// check whether registration debugging is enabled
bool is_registration_debugging_enabled() { return ref_registration_debugging_enabled(); }
...
bool is_registration_enabled() { return ref_info().registration_enabled; }
...
void unregister_all_objects() { ref_object_collection().unregister_all_objects(); }

/// access to number of permanently registered objects
unsigned get_nr_permanently_registered_objects() { return (unsigned)ref_object_collection().objects.size(); }
...
bool is_permanent_registration_enabled() { return ref_info().permanent_registration; }
...
bool is_registration_event_cleanup_enabled() { return ref_info().registration_event_cleanup; }
...
named_ptr find_object_by_name(const std::string &name) { return ref_object_collection().find_object_by_name(name); }
...
bool process_config_file(const std::string &_file_name) { return process_config_file_ext(_file_name); }
...
std::string test::get_test_name() const { return test_name; }

bool test::exec_test() const { return test_func(); }

int cgv::base::test::nr_failed = 0;

std::string test::get_type_name() const { return "test"; }
...
std::string factory::get_object_options() const { return object_options; }

/// support creation of object by setting create property to true
std::string factory::get_property_declarations() { return "create:bool"; }
...
const std::string &factory::get_created_type_name() const { return created_type_name; }
/// return whether the factory can only generate one instance of the given type
bool factory::is_singleton_factory() const { return is_singleton; }

/// return pointer to singleton
base_ptr factory::get_singleton() const { return singleton; }
/// release the singleton pointer
void factory::release_singleton() { singleton.clear(); }
...
bool process_command(const command_info &info) { return process_command_ext(info); }
