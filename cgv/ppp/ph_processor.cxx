			...
			while (i < j) {
				...
				case CT_RAND:
				{
					...
					if (minval.is_double() || maxval.is_double()) {
						std::uniform_real_distribution<double> D(minval.get_double(), maxval.get_double());
						double d = D(R);
						ref_variable(str.is_name() ? str.get_name() : str.get_str()) = variant(d);
					}
					else {
						std::uniform_int_distribution<int> D(minval.get_int(), maxval.get_int());
						int i = D(R);
				...
				case CT_SCAN_INCLUDES:
				{
					...
					variant& v_str = ref_variable(v2.get_name());
					if (!v_str.is_str()) {
