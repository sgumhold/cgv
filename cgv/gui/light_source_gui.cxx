...
struct light_source_gui_creator : public gui_creator
{
		...
		const std::string& gui_type, const std::string& options, bool* toggles)
	{
		...
		if(p->add_tree_node("Colors", toggles[1], 2)){
			p->align("\a");
		...
		if (light.get_type() == LT_DIRECTIONAL) {
			if(p->add_tree_node("Direction", toggles[2], 2)){
				p->align("\a");
			...
			if(p->add_tree_node("Position", toggles[3], 2)){
				p->align("\a");
		...
		if(p->add_tree_node("Attenuation", toggles[4], 2)){
			p->align("\a");
		...
		if(p->add_tree_node("Spot Parameters", toggles[5], 2)){
			p->align("\a");
...
cgv::gui::gui_creator_registration<light_source_gui_creator> light_source_gc_reg;
