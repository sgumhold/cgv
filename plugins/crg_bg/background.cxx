...
void background::on_set(void* member_ptr)
{
	if (member_ptr == &enable) {
		auto* ctx_ptr = get_context();
		if (ctx_ptr)
			sync_render_flags(*ctx_ptr);
	}
	update_member(member_ptr);
	post_redraw();
}

void background::stream_help(std::ostream& os)
{
	os << "background: <Shift + F12> to toggle mode";
}

bool background::handle(event& e)
{
	if (e.get_kind() != EID_KEY)
		return false;

	auto& ke = dynamic_cast<key_event&>(e);		
...
bool background::init(context& ctx)
{
...
void background::clear(context& ctx)
{
	prog.destruct(ctx);
}

void background::init_frame(context& ctx)
{
	if (!enable)
		return;
...
void background::sync_render_flags(context& ctx) const {
	if(enable)
