...
struct CGV_API fltk_gui_group : public fltk_base
{
	...
	virtual void finalize_new_element(cgv::gui::gui_group_ptr ggp, const std::string& align, cgv::base::base_ptr child);
	/// remove the given child, if it appears several times, remove al instances. Return the number of removed children
	virtual unsigned int remove_child(cgv::gui::gui_group_ptr ggp, cgv::base::base_ptr child);
	/// remove all children
